...
이 문서는 Java로 구현된 간단한 회원 관리 시스템 예제를 통해 객체 지향 프로그래밍의 핵심 개념을 이해하고 복습하는 것을 목표로 합니다.

## 1. 프로젝트 개요 및 요구사항

먼저, 우리가 만들고자 하는 프로그램의 목표를 명확히 합니다.
...
3.  **`MemberRepository`**: `Controller`의 요청에 따라 실제 회원 데이터를 관리(추가, 조회, 수정, 삭제)합니다. 데이터베이스 역할을 합니다.
4.  **`Member`**: `Repository`가 관리하는 데이터의 기본 단위입니다. 한 명의 회원 정보를 담는 객체의 설계도입니다.
...
-   **Controller**는 사용자의 요청을 해석하여 **Repository**에게 데이터 처리를 지시합니다.
-   **Repository**는 **Member** 객체들을 배열에 담아 관리합니다.

## 3. 코드 상세 분석 (실행 흐름 순)

이제 실제 코드를 보며 각 클래스의 역할과 주요 개념을 살펴보겠습니다.
...
-   **역할**: 사용자와의 상호작용(입력/출력)을 담당합니다. 사용자의 요청을 받아 `MemberRepository`에 전달하는 '중간 관리자' 역할을 합니다.
-   **핵심 개념**:
    -   **객체의 협력과 위임**: `Controller`는 데이터 관리를 직접 하지 않고, `MemberRepository` 객체(`mr`)에게 그 책임을 위임합니다.
    -   **의존 관계**: `MemberController`는 `MemberRepository`의 기능이 필요하므로, 필드(멤버 변수)로 `MemberRepository` 객체를 가집니다. 이를 '의존 관계'라고 합니다.
    -   **메서드 분리**: `signUp()`, `showAllMembers()`처럼 각 기능을 별도의 메서드로 만들어 코드의 가독성과 재사용성을 높였습니다.
...
-   **역할**: 모든 회원 데이터(`Member` 객체 배열)를 실제로 저장하고 관리합니다. 데이터에 대한 CRUD(생성, 조회, 수정, 삭제) 책임을 가집니다.
-   **핵심 개념**:
    -   **데이터 저장소**: `Member[] memberList` 배열이 데이터베이스와 같은 역할을 합니다.
    -   **캡슐화(Encapsulation)**: 데이터(`memberList`)를 `private`으로 숨기고, 외부에서는 `public` 메서드(`addMember`, `getMembers` 등)를 통해서만 데이터에 접근하도록 하여 데이터의 무결성을 지킵니다.
    -   **배열 관리**: `addMember` 메서드에서 보이듯, 배열의 크기를 동적으로 늘리기 위해 기존 배열보다 큰 새 배열을 만들고 데이터를 복사하는 방식을 사용합니다.
...
-   **역할**:
    -   **`Member.java`**: 회원 한 명의 정보를 담기 위한 '틀' 또는 '설계도'입니다. 이 클래스를 통해 생성된 객체(`인스턴스`) 하나가 회원 한 명을 의미합니다. 이러한 객체를 **VO(Value Object)** 또는 **DTO(Data Transfer Object)** 라고 부르기도 합니다.
    -   **`Gender.java`**: 성별처럼 'MALE', 'FEMALE' 같이 정해진 값만 사용해야 할 때 실수를 방지하기 위해 사용하는 **열거형(Enum)**입니다.
-   **핵심 개념**:
    -   **클래스와 객체**: `Member`는 클래스(설계도), `new Member(...)`로 만들어진 결과물은 객체(실체)입니다.
    -   **필드(Field)**: 객체가 가질 데이터(속성)입니다. (예: `email`, `password`)
    -   **생성자(Constructor)**: 객체가 `new` 키워드로 생성될 때 필드를 초기화하는 역할을 합니다.
    -   **메서드(Method)**: 객체가 수행할 수 있는 동작입니다. (예: `inform()`)
    -   **`toString()` 오버라이딩**: 객체를 `System.out.println()`으로 출력할 때, 주소값 대신 필드 정보를 보기 좋게 출력하기 위해 재정의합니다.
...
이 예제를 통해 배운 객체 지향의 핵심 개념은 다음과 같습니다.

1.  **클래스(Class)와 객체(Object)**
    -   `Member.java`는 **클래스**(설계도)이고, `new Member(...)`로 생성된 각각의 회원 정보는 **객체**(실체)입니다.

2.  **캡슐화(Encapsulation)**
    -   `MemberRepository`가 `memberList` 배열을 직접 노출하지 않고 `getMembers()`, `addMember()` 같은 메서드를 통해 접근하게 하는 것입니다. 데이터와 데이터를 처리하는 로직을 하나로 묶어 외부의 잘못된 사용을 막습니다.

3.  **역할과 책임(Role and Responsibility)**
    -   `Controller`는 입출력, `Repository`는 데이터 관리, `Member`는 정보 저장 등 각 클래스가 명확한 **역할**과 **책임**을 가집니다. 이를 통해 코드를 이해하고 수정하기 쉬워집니다.

4.  **협력(Collaboration)**
    -   객체는 혼자 모든 것을 처리하지 않습니다. `Controller`가 `Repository`의 기능을 호출하고, `Repository`가 `Member` 객체를 사용하는 것처럼 객체들은 서로의 기능을 요청하며 **협력**합니다.
